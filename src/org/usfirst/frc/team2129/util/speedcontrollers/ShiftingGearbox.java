...
//FIXME: not used
public class ShiftingGearbox implements SpeedController, Team2129GlobalInterface {
	...
	private DoubleSolenoid dShifter;
	private boolean useDShifter;

	private String nickname;

	private double twoMotorThreshold;
	private double threeMotorThreshold;
	private double hiSpeedThreshold;
	private boolean solenoidInverted;
	private boolean enableShifting;

	private boolean logging;

	public ShiftingGearbox(SpeedController motor1, SpeedController motor2, SpeedController motor3, double threshold2, double threshold3, double shiftThreshold,
			boolean solenoidInverted, boolean enableShifting, String nickname) {
		this.motor1 = motor1;
		this.motor2 = motor2;
		this.motor3 = motor3;
	...
	public ShiftingGearbox(SpeedController rightGearboxMotor1, SpeedController rightGearboxMotor2, SpeedController rightGearboxMotor3,
			DoubleSolenoid rightGearboxShifter, double threshold2, double threshold3, double shiftThreshold, boolean solenoidInverted2, boolean enableShifting2,
			String nickname2) {
		this(rightGearboxMotor1, rightGearboxMotor2, rightGearboxMotor3, threshold2, threshold3, shiftThreshold, solenoidInverted2, enableShifting2, nickname2);
		this.dShifter = rightGearboxShifter;
		this.useDShifter = true;
	}

	public ShiftingGearbox(SpeedController rightGearboxMotor1, SpeedController rightGearboxMotor2, SpeedController rightGearboxMotor3,
			Solenoid rightGearboxShifter, double threshold2, double threshold3, double shiftThreshold, boolean solenoidInverted2, boolean enableShifting2,
			String nickname2) {
		this(rightGearboxMotor1, rightGearboxMotor2, rightGearboxMotor3, threshold2, threshold3, shiftThreshold, solenoidInverted2, enableShifting2, nickname2);
	...
	public void setGearBool(boolean gear) {
		if (useDShifter) {
			if (solenoidInverted ? !gear : gear) {
				dShifter.set(DoubleSolenoid.Value.kForward);
			} else {
				dShifter.set(DoubleSolenoid.Value.kReverse);
			}
		} else {
			shifter.set(solenoidInverted ? !gear : gear);
		}
	}
	...
	public void putNumber(String name, double number) {
		if (logging)
			setSmartDashboard(nickname + "::" + name, number);
	}
